...
import { Plus, Search, ChevronLeft, Edit2 } from "lucide-react";
...
const Orcamentos = () => {
  ...
  const filtered = orcamentos.filter((o: any) => {
    ...
    return !term || o.clientes?.nome?.toLowerCase().includes(term) || String(o.numero).includes(term) || o.descricao?.toLowerCase().includes(term);
  });
                ...
                {filtered.map((orc: any) => (
                  <tr key={orc.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${statusColor[orc.status] || "bg-muted text-muted-foreground"}`}>
        ...
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            ...
            <div>
              ...
              <Input value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} className="mt-1" placeholder="Ex: Janela 2 folhas" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input type="date" value={form.validade} onChange={(e) => setForm({ ...form, validade: e.target.value })} className="mt-1" />
              </div>
            </div>
            <div>
