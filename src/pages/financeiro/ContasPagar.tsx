...
const ContasPagar = () => {
  const { user } = useAuth();
  const { data: contas = [], isLoading } = useSupabaseQuery("contas_pagar", {
    orderBy: { column: "data_vencimento", ascending: true },
  });
  const insertMutation = useSupabaseInsert("contas_pagar");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [form, setForm] = useState({ fornecedor: "", descricao: "", valor: "", data_vencimento: "" });
        ...
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            <div>
              ...
              <Input value={form.fornecedor} onChange={(e) => setForm({ ...form, fornecedor: e.target.value })} className="mt-1" />
            </div>
            <div>
